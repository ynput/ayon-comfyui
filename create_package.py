...
import additional_tooling as tools
import package
...
def get_client_zip_content(
    ...
    # MOD (@Sas): integrate comfyui plugin into package
    if (
        package.comfyui_plugin_fetch
        and package.comfyui_plugin
        and Path(package.comfyui_plugin).exists()
    ):
        log.info("Including ComfyUI plugin into client package.")
        tools.include_ayon_comfyui_plugin(package.comfyui_plugin)
